...
image: assets/posts/flocker.png
...
<blockquote>Flocker is an open-source container data volume manager for your Dockerized application. It gives ops teams the tools they need to run containerized stateful services like databases in production.</blockquote>
...
**_Please note — You should also transfer SSH keys between all of the nodes as mentioned at the top of this post._**
...
Now add the ClusterHQ yum repo and install the packages needed to run Flocker;
    ...
    yum install -y <a href="https://clusterhq-archive.s3.amazonaws.com/centos/clusterhq-release%24%28rpm" target="_blank" data-href="https://clusterhq-archive.s3.amazonaws.com/centos/clusterhq-release$%28rpm">https://clusterhq-archive.s3.amazonaws.com/centos/clusterhq-release$(rpm</a> -E %dist).noarch.rpm
...
**_The installation will take several minutes, this is normal and there is nothing to worry about if the process appears to hang during install._**

    
    sync
    yum install -y <a href="https://s3.amazonaws.com/archive.zfsonlinux.org/epel/zfs-release.el7.noarch.rpm" target="_blank" data-href="https://s3.amazonaws.com/archive.zfsonlinux.org/epel/zfs-release.el7.noarch.rpm">https://s3.amazonaws.com/archive.zfsonlinux.org/epel/zfs-release.el7.noarch.rpm</a>
    yum install -y zfs
...
Now on each of the **storage nodes** its time to create the ZFS pool, we will be calling the pool _flocker_;
...
First of all we need to create the certificate authority, all of the other certificates will be signed using this one. We will be calling the cluster _flocker_;
...
The UID in this run was “_0ed63f10–7065–4d37-af7c-6128cb5c072f_”. Now we have the first node certificate and key we need to copy it to the first of our **storage nodes** along with the cluster.crt and set the right permissions on the key file;
...
As you can see from the following, this time we got a UID of “_f939152e-0ec0–4d7c-ae74–37c899648dca_”;
...
**_Please note: It is important that each node has its own certificate, if you upload the same certificate to more than one node you will cause a conflict with the Flocker control service which will only register a single storage node._**
...
Make a note of the “CN”, which in the example above is _user-flocker-russ_. Next up we need to create a .p12 certificate which can be understood by OSX, make sure you enter an export password when prompted;
...
First of all lets make a connection to the Flocker Control API and list the nodes. The following command will connect using the API certificate we imported into the Keychain, this is called by using the CN which was _user-flocker-russ_;

    
    curl — cacert ~/Documents/Projects/Flocker/cluster.crt — cert “user-flocker-russ” <a href="https://control.mydomain.com:4523/v1/state/nodes" target="_blank" data-href="https://control.mydomain.com:4523/v1/state/nodes">https://control.mydomain.com:4523/v1/state/nodes</a>
...
Now you have written to “_wibble:/data_” you can launch another cotainer which remounts “_wibble:/data_” and prints the content of “_/data/file.txt_” to the screen;
