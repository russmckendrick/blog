...
export const LOCKUP_RATIO = lockup.width / lockup.height;
...
/**
 * Resolves a site-root URL path ("/assets/…/Album.jpg") to the file under
 * public/ that serves it.
 *
 * Kept separate from resolveCoverPath because these two take different things
 * that look identical: a leading slash means "site root" here and "filesystem
 * root" there, so feeding one of these paths to resolveCoverPath silently looks
 * for the file at the top of the disk and finds nothing.
 */
export function resolvePublicAsset(urlPath: string): string {
...
export function headlineSize(length: number, ladder: number[]): number {
...
export function MetaLine({ items, color }: { items: string[]; color: string }) {
  return (
    <div
      style={{
        display: "flex",
