...
cover:
  image: "../../assets/2024-07-27-creating-an-azure-storage-account-sas-token-from-a-logic-app/blog-cover-2024-07-27-creating-an-azure-storage-account-sas-token-from-a-logic-app.png"
draft: false
showToc: true
tags:
  - "Azure"
...
<LinkPreview id="https://learn.microsoft.com/en-us/connectors/azureblob/#create-sas-uri-by-path-(v2)" />
...
```json frame="code" title="LocalFunctionProj/getSAS/run.ps1 (JSON)" showLineNumbers
{
  "code": "ExportFailed",
...
```text frame="terminal" title="Set some variables"
...
```text frame="terminal" title="Add the resource group using the Azure CLI"
az group create \
...
```text frame="terminal" title="Deploy the Storage Account using the Azure CLI"
...
```text frame="terminal" title="Deploy the Function App using the Azure CLI"
...
```text frame="terminal" title="Deploy to the Function App using the Azure CLI"
...
```powershell frame="code" title="LocalFunctionProj/requirements.psd1 (POWERSHELL)" showLineNumbers
@{
    'Az' = '12.*'
}
```
...
```powershell frame="code" title="LocalFunctionProj/getSAS/run.ps1 (POWESHELL)" showLineNumbers
...
```json frame="code" title="The JSON body (JSON)" showLineNumbers
{
...
```json frame="code" title="The JSON result (JSON)" showLineNumbers
{
  "message": "SAS Token generated successfully",
...
```json frame="code" title="Getting the information into the LogicApp (JSON)" showLineNumbers
"Parse_JSON_(SAS)": {
    "inputs": {
...
```json frame="code" title="Calling the third-part API (JSON)" showLineNumbers
...
<Audio mp3="/assets/2024-07-27-creating-an-azure-storage-account-sas-token-from-a-logic-app/2024-07-27-creating-an-azure-storage-account-sas-token-from-a-logic-app.mp3" />
