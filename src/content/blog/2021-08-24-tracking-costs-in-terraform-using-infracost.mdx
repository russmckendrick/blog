...
date: 2021-08-24
cover:
  image: "../../assets/2021-08-24-tracking-costs-in-terraform-using-infracost/blog-cover-2021-08-24-tracking-costs-in-terraform-using-infracost.png"
draft: false
showToc: true
tags:
  - "Terraform"
...
```bash frame="terminal" title="Install Infracost on macOS"
``` terminfo
...
```bash frame="terminal" title="Register for an API key"
``` terminfo
infracost register
```
```
...
```bash frame="terminal" title="Run some Terraform commands"
``` terminfo
terraform init
...
```bash frame="terminal" title="Run Infracost for the first time"
``` terminfo
...
```bash frame="terminal" title="Check the differences"
``` terminfo
...
```bash frame="terminal" title="Run Terraform Destroy"
``` terminfo
terraform destroy
```
```
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/01-update-variable.png" alt="Adding the INFRACOST_API_KEY variable" />
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/06-skipping.png" alt="Nothing to do here" />
...
```bash frame="terminal" title="Set $past_total_monthly_cost"
``` bash
past_total_monthly_cost=$(jq '[.projects[].pastBreakdown.totalMonthlyCost | select (.!=null) | tonumber] | add' $(System.DefaultWorkingDirectory)/output/cost.json)
```
```
...
```bash frame="terminal" title="Set $total_monthly_cost"
``` bash
total_monthly_cost=$(jq '[.projects[].breakdown.totalMonthlyCost | select (.!=null) | tonumber] | add' $(System.DefaultWorkingDirectory)/output/cost.json)
```
```
...
```bash frame="terminal" title="Set $diff_cost"
``` bash
diff_cost=$(jq '[.projects[].diff.totalMonthlyCost | select (.!=null) | tonumber] | add' $(System.DefaultWorkingDirectory)/output/cost.json)
```
```
...
```bash frame="terminal" title="Set $percentage_threshold"
``` bash
percentage_threshold=$(cost_increase_alert_percentage)
```
```
...
```bash frame="terminal" title="Set $percent"
``` bash
if [ $(echo "$past_total_monthly_cost > 0" | bc -l) = 1 ] && [ $(echo "$total_monthly_cost > 0" | bc -l) = 1 ]; then
  percent=$(echo "scale=6; $total_monthly_cost / $past_total_monthly_cost * 100 - 100" | bc)
fi
...
```bash frame="terminal" title="Set $percent to 0"
``` bash
if [ $(echo "$past_total_monthly_cost <= 0" | bc -l) = 1 ] && [ $(echo "$total_monthly_cost <= 0" | bc -l) = 1 ]; then
  percent=0
fi
...
```bash frame="terminal" title="Decide what to do"
``` bash
  if [ -z "$percent" ]; then
    echo "##vso[task.logissue type=warning]💰 - ℹ️ No previous cost data has been detected"
  elif [ $(echo "$percent > $percentage_threshold" | bc -l) = 1 ]; then
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/02-first-run.png" alt="First run" />
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/03-no-change.png" alt="Second run" />
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/04-increase-the-sku.png" alt="Update the SKU" />
...
<Img src="/assets/2021-08-24-tracking-costs-in-terraform-using-infracost/images/05-decrease-the-sku.png" alt="Undo the change to the SKU" />
