...
cover:
  image: "../../assets/2021-08-28-rocky-linux-packer/blog-cover-2021-08-28-rocky-linux-packer.png"
draft: false
showToc: true
avatar: "nerd"
tags:
...
```bash frame="terminal" title="Set some variables"
``` hcl
variable "headless" {
  type    = string
  default = "true"
...
```bash frame="terminal" title="Virtualbox"
``` hcl
...
```bash frame="terminal" title="VMWare"
``` hcl
...
```bash frame="terminal" title="Build"
``` hcl
build {
...
```bash frame="terminal" title="Build the Virtualbox VM"
``` terminfo
...
```bash frame="terminal" title="Build VMware VM"
``` terminfo
...
```bash frame="terminal" title="Build both VMs"
``` terminfo
...
```bash frame="terminal" title="The Vagrantfile"
``` ruby
# -*- mode: ruby -*-
# vi: set ft=ruby :
...
```bash frame="terminal" title="Launch the virtual machine and SSH to it"
``` terminfo
vagrant up
vagrant ssh
```
...
```bash frame="terminal" title="Stop or Destroy"
``` terminfo
vagrant stop
vagrant destroy
```
