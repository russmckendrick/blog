...
cover:
  image: "../../assets/2024-07-07-avd-kql-query-notes/blog-cover-2024-07-07-avd-kql-query-notes.png"
draft: false
showToc: true
tags:
  - "azure"
...
```kql frame="code" title="Find out all the IP addresses in the last 30 days (KQL)" showLineNumbers
let daysAgo = 30d;
WVDConnections
...
```kql frame="code" title="Find out all Users and their IP addresses in the last 30 days (KQL)" showLineNumbers
let daysAgo = 30d;
WVDConnections
| where TimeGenerated > ago(daysAgo)
| summarize NumberOfConnections = count() by UserName, ClientIPAddress
...
```kql frame="code" title="Find out the IP addresses of where a user is connecting from (KQL)" showLineNumbers
...
```kql frame="code" title="Get all errors for all users from a single IP address (KQL)" showLineNumbers
...
```kql frame="code" title="Total session time (KQL)" showLineNumbers
let daysAgo = 31d;
WVDConnections
| where TimeGenerated > ago(daysAgo)
...
```kql frame="code" title="Total number of sessions per host pool (KQL)" showLineNumbers
let daysAgo = 31d;
WVDConnections 
| where TimeGenerated > ago(daysAgo)
| where State == "Connected" 
...
```kql frame="code" title="Total Sessions (KQL)" showLineNumbers
let daysAgo = 1d;
WVDConnections
...
```kql frame="code" title="Errors per host pool (KQL)" showLineNumbers
let daysAgo = 1d;
WVDErrors
| where TimeGenerated > ago(daysAgo) 
