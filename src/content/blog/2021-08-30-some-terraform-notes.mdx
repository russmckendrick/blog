...
cover:
  image: "../../assets/2021-08-30-some-terraform-notes/blog-cover-2021-08-30-some-terraform-notes.png"
draft: false
showToc: true
tags:
  - "Terraform"
...
```hcl frame="code" title="Create the example resources (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 1/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Use azurerm_monitor_diagnostic_categories (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 2/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Add the Diagnostic Settings (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 3/15"
```hcl {linenos=true}
...
```hcl frame="code" title="The output (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 4/15"
```hcl {linenos=true}
logs = toset([
  "VMProtectionAlerts",
])
...
```hcl frame="code" title="Setting a 30 day expiry (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 5/15"
```hcl {linenos=true}
terraform {
...
```hcl frame="code" title="The output (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 6/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Hostpool example (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 7/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Key Vault Secret example (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 8/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Create an Azure Automation Account and Runbook (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 9/15"
```hcl {linenos=true}
terraform {
  required_version = ">= 1.0.0"
  required_providers {
...
```hcl frame="code" title="Generate some stuff (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 10/15"
```hcl {linenos=true}
...
```hcl frame="code" title="Set the local variable (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 11/15"
```hcl {linenos=true}
locals {
...
```hcl frame="code" title="Generate some stuff (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 12/15"
```hcl {linenos=true}
...
```hcl frame="code" title="The output (HCL)"
```bash frame="terminal" title="Some Terraform Azure Notes 13/15"
```hcl {linenos=true}
...
```bash frame="terminal" title="The output"
``` terminfo
...
```bash frame="terminal" title="The Job ID"
```json {linenos=true}
{
  "JobIds": [
