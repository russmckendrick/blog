...
You might think disabling lifecycle scripts entirely would prevent this attack:

```bash
pnpm install --ignore-scripts
```

However, many legitimate packages **require** lifecycle scripts to function. In my case:
...
Using `--ignore-scripts` would break my build entirely. The better protection is the combination of **lockfiles** and **`--frozen-lockfile`** - these ensure you only install exactly what you've previously vetted, not whatever the latest (potentially compromised) version happens to be.
