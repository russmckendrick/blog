...
Image calls are delegated to swappable backends in `scripts/lib/image-backends/`. The primary compose backend comes from `settings.cover_backend` in `scripts/tunes-config.yaml` and defaults to `gpt-image-2-5` (OpenAI `gpt-image-2.5/sunburst/edit`, 2560×1440 at high quality). Composing is the only image stage; the generated image ships as-is.
