...
Use this for direct Tunes cover generation. The script selects the ~7-8 strongest album covers, then runs two deliberately separate OpenAI stages. A factual vision stage records a description, signature non-text motif, original medium, visible people, and palette for each source; sources with people get a code-appended reference map in the final prompt so faces are copied from the numbered sleeve rather than invented, and references are uploaded border-trimmed at 1280px (`TUNES_COVER_REFERENCE_SIZE`). A freeform art-director stage receives those summaries, an optional preconfigured author hint, recent do-not-repeat concepts, and the media used on recent covers—no post content. It chooses the medium, scene, viewpoint, composition, lighting, palette, and final creative prompt without a preset style catalogue, leaning photographic by default. The chosen `medium` is recorded in the sidecar and history; non-photographic media are refused on subsequent weeks (photographic treatments never are), which is what stops the run of near-identical paper-collage dioramas that concepts alone never caught.
...
Image calls are delegated to swappable backends in `scripts/lib/image-backends/`. The primary compose backend comes from `settings.cover_backend` in `scripts/tunes-config.yaml` and defaults to `gpt-image-2-5` (OpenAI `gpt-image-2.5/sunburst/edit`, 2560×1440 at `xhigh` quality - the backend's default since 21 Sep 2026, when it visibly improved sleeve likenesses; `GPT_IMAGE_2_5_QUALITY` overrides it). Composing is the only image stage; the generated image ships as-is.
