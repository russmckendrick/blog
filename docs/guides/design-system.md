...
**Contents block** (`TableOfContents.astro`): rendered between hero and prose whenever `showToc` is set and headings exist, at every viewport width — a native `<details>` with hairline rules top and bottom, a chevron + "Contents" summary (15px/600), and 15px mist entries (ink on hover) anchor-linking to the post's h2/h3s. Posts with four or more entries open by default; the reader's own toggle is remembered per post.
