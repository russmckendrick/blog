...
- `.action-pill` — the bordered article-tool button: 13px/600 sans, 8px radius, hairline border on `--color-surface`, filling to `--paper-well` on hover. Shared by the Use with AI trigger, Suggest edits and RSS so the three read as one control family; a leading `.action-pill-icon` glyph sits at 1rem in mist, leaving the Use with AI sparkle (in accent) the only one that draws the eye. Width is the container's call — the rail and narrow storybar stretch them, the inline storybar lets them size to content.
...
`BlogPost.astro`: balanced sans title → mist standfirst → single-line byline — a 36px circular **tag-based avatar** (the illustrated set in `public/images/avatars/`, first mapped tag wins via `TAG_AVATAR_MAP` in `src/consts.ts`, `anon.svg` fallback; tunes use the AI author's avatar) beside `Russ McKendrick · 9 min read · 19 Jul 2026` (name in ink, the rest mist). Then the hairline **storybar** (`StoryBar.astro`): tags left, three `.action-pill` buttons right — `[✦ Use with AI ▾]`, `[ GitHub Suggest edits]`, `[ RSS]` — with hairline rules above and below. Below 640px the actions take a block of their own: the trigger spans the full width, the two links split the row beneath 50/50 (a post with no `editUrl` lets RSS span rather than sit at half width). It renders below 1200px only — above that the article rail carries the same content in the right margin, so the two never appear together. Hero figure at column width, 4px radius. A 2px accent **reading-progress rule** fixed to the top viewport edge runs on article pages only — no gradient, no glow.

**Article rail** (`ArticleRail.astro`): an always-open, sticky utility column in the right margin beside the centred article column on viewports ≥1200px. Hairline left rule and three headed sections (13px/600 heads): **Contents** — 13px mist entries (ink on hover), nested h3s indented, the section currently in view highlighted in ink 600 via IntersectionObserver, rendered only when `showToc` is set (tunes cap at h2s); **Tags** — small tinted pills (`.tag-editorial--sm`); **Actions** — three `.action-pill` buttons stacked at the rail's full width: the Use with AI menu, then Suggest edits and RSS. Below 1200px the rail doesn't render — the storybar under the byline picks up its Tags and Actions, tags also remain at the article foot, and headings plus the reading-progress rule carry wayfinding. Contents stays rail-only at every width. Feed-row meta lines show up to three `.tag-editorial--sm` pills.

**Use with AI** (`UseWithAI.astro`): the one card in the system, and a deliberate exception — a bordered pill trigger (sparkle glyph in accent, 13px/600 label, rotating chevron) opening a 320px menu with a 2px ink border, 14px radius and six rows, each a 36px hairline icon tile beside a 14px/600 title and a 13px mist subtitle: Copy as Markdown, View as Markdown, then a hairline and Open in ChatGPT / Claude / Perplexity / Gemini. The provider links carry a prefilled prompt pointing at the post's canonical URL — not the `.md` twin, which ChatGPT's reader refuses on content type; the worker negotiates markdown at the canonical URL anyway. Copy fetches the twin to the clipboard and swaps its own subtitle to confirm; both markdown rows need a built site, so they 404 under `pnpm run dev`. The fourth row is Google AI Mode rather than Gemini: the Gemini web app ignores `?q=` and would open an empty box, while AI Mode is the same model reached through Search, which does take a query (`udm=50`). It renders in both the storybar and the rail — the two are viewport-exclusive, so only one is ever on screen. The card is a `popover`, placed in viewport coordinates by JS so the rail's `overflow-y: auto` cannot clip it and a 320px card beside the 220px rail stays inside the viewport.
