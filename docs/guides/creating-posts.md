...
With `showToc: true`, the headings render as the **Contents rail** - an always-open, sticky list in the right margin beside the article column on wide viewports (≥1200px), with the section currently being read highlighted. On narrower screens no table of contents renders; there is no inline ToC. The rail's other two sections do follow you down: below 1200px the **storybar** (`StoryBar.astro`) renders under the byline with the post's tags on the left and the **Use with AI** menu, `Suggest edits` and `RSS` on the right as pill buttons (stacked full-width below 640px). The Use with AI menu (`UseWithAI.astro`) copies or opens the post's markdown twin, or hands it to ChatGPT, Claude, Perplexity or Google AI Mode with a prefilled prompt — note the twins are generated postbuild, so those actions only work against a built site, not `pnpm run dev`.
