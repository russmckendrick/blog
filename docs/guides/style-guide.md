...
- The brand lockup on the left — `Logo.astro`, one self-contained inline SVG: the iMac mark + `russ.cloud` in baked Poppins outlines (heavy ink "russ", light mist ".cloud") with a blinking block cursor on the baseline (1.1s square wave, steady under reduced motion). The dot and cursor take the mark's own colours — screen blue `#35495E` on paper, base grey `#BDC3C7` in the Night edition — and "russ"/".cloud" ride the ink/mist tokens, so the lockup follows the theme on its own. On the right, five links (Tunes · Books · Tags · Archive · About) in mist, ink on hover, each led by a 15px hairline glyph from `Icon.astro` sitting at 62% opacity so the label still carries the row — glyph and label reach full ink together on hover — then a 16px vertical hairline (`.masthead-divider`) separating the utility icons: the search trigger and the icon-only theme toggle. Nav items and their glyphs are declared together in `MASTHEAD_ITEMS` (`src/consts.ts`); add the icon name there, not in the template.
