...
| `/tunes/artist/` and `/tunes/artist/{slug}/[page]/` | Browse weekly posts by featured artist | `src/data/tunes-index.json` |
| `/tunes/album/` and `/tunes/album/{slug}/` | Browse by featured album, with `MusicRecording` schema | `src/data/tunes-index.json` |
...
`src/data/tunes-index.json` is regenerated by `scripts/build-tunes-index.js` (wired into `pnpm run prebuild` and the end of `pnpm run tunes`). It parses the "Top Albums" section out of every weekly tunes post into a sorted artist/album lookup.
